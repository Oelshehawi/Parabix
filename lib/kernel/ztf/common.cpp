...
LengthGroupParameters::LengthGroupParameters(BuilderRef b, EncodingInfo encodingScheme, unsigned groupNo, unsigned numSym) :
    ...
    LAST_SUFFIX_MASK(b->getSize((1UL << encodingScheme.lastSuffixHashBits(numSym, groupNo)) - 1UL)),
    SEC_LAST_SUFFIX_MASK(b->getSize((1UL << encodingScheme.secLastSuffixHashBits(numSym, groupNo)) - 1UL)),
...
//    symLengthTy(b->getIntNTy(1UL << (uint64_t)(groupInfo.hi + 3UL))),
//    symLengthPtrTy(symLengthTy->getPointerTo()),
    ...
    TABLE_MASK(b->getSize((1U << encodingScheme.tableSizeBits(groupNo, numSym)) - 1UL)),
    ...
    TABLE_IDX_MASK(b->getSize((1U << (8UL * groupInfo.encoding_bytes)) - 1UL)),
    FREQ_TABLE_MASK(b->getSize((1UL << (18UL - groupNo)) - 1UL)),
...
std::vector<Value *> initializeCompressionMasks2_init(BuilderRef b,
                                                ScanWordParameters & sw,
                                                Constant * sz_BLOCKS_PER_STRIDE,
                                                unsigned maskCount,
                                                Value * absBlockOffset,
    ...
    Value * hashMaskCurPtr = b->CreateBitCast(b->getRawOutputPointer("hashMarks", blockPos), bitBlockPtrTy);
    Value * secHashMaskCurPtr = b->CreateBitCast(b->getRawOutputPointer("secHashMarks", blockPos), bitBlockPtrTy);
    b->CreateBlockAlignedStore(b->allZeroes(), hashMaskCurPtr);
